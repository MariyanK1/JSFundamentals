...
Next he jumps again with a length of 2 and goes outside the neighborhood, so he goes back to the first 
house (index 0) and again decreases the needed hearts there: [4, 6, 4].
...
function solve(input = []) {
  ...
  let command;
  let currIndex = 0;
  while ((command = input.shift()) !== "Love!") {
    ...
    let success = true;
    let count = 0;
    for (const houses of neighborHood) {
        if (houses !== 0) {
            success = false;
            count++
        }
